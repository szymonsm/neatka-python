"""
NEAT Parameter Sweep Script for Cart-Pole Balancing
...
RESULTS_BASE_DIR = 'examples/single-pole-balancing/parameter_sweep_results'
...
def run_experiment(params, seed, results_dir):
    ...
    base_config_path = os.path.join('examples', 'single-pole-balancing', f'config-{net_type}')
    ...
    cmd = [
        'python', 
        os.path.join('examples', 'single-pole-balancing', 'evolve.py'),
        '--net-type', net_type,
        '--seed', str(seed),
        '--config', config_path,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run NEAT parameter sweep")
